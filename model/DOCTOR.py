...
class MEAM(torch.nn.Module):
    def __init__(self, dataset):
        ...
        self.mlp_ocr_pattern = nn.Sequential(nn.Linear(4096, 2048), nn.ReLU(), nn.Dropout(0.1),
                                             torch.nn.Linear(2048, 512), torch.nn.ReLU(), nn.Dropout(0.1),
...
class DCTOR_Model(torch.nn.Module):
    ...
    def forward(self, dg, diffusion, global_step=0, sma_count=0, **kwargs):
        output_msam = self.content_branch(**kwargs)
        output_meam = self.editing_branch(**kwargs)
        if dg:
            output_dg, dg_loss = self.dg_model(global_step, sma_count, **kwargs)
        if diffusion:
            output_diffusion = self.diffusion(**kwargs)

        if dg and diffusion:
            output = output_msam * self.tanh(output_meam) * output_dg * output_diffusion
            return output, output_msam, output_meam, dg_loss, output_diffusion
        elif dg:
            output = output_msam * self.tanh(output_meam) * output_dg
            return output, output_msam, output_meam, dg_loss
        elif diffusion:
            output = output_msam * self.tanh(output_meam) * output_diffusion
            return output, output_msam, output_meam, output_diffusion
        else:
